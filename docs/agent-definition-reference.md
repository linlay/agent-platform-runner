...
- 默认最小集：默认挂载 `/workspace`、`/root`、`/skills`、`/pan`、`/agent`、`/owner`、`/memory`；其余平台目录仍按需显式恢复。
- agent 就近原则：当前 agent 默认挂载其自身目录到 `/agent`；若 `agents/<agentKey>/` 缺失，直接 fail-fast。
- 本地优先原则：目录化 agent 在 `RUN/AGENT` 级别会优先把 `agents/<agentKey>/skills` 挂到 `/skills`；没有本地 skills 时才回落到共享 market。
- 默认安全模式：`/skills`、`/agent`、`/owner`、`/memory` 默认只读；`/workspace`、`/root`、`/pan` 默认读写。
- 自动准备原则：`/owner` 对应的 `OWNER_DIR` 不存在时会自动创建；`/memory` 对应的 `MEMORY_DIR/<agentKey>` 不存在时会自动创建。
- 按需显式原则：`/models`、`/tools`、`/agents`、`/viewports`、`/teams`、`/schedules`、`/mcp-servers`、`/providers`、`/chats`、`/skills-market` 仅能通过 `sandboxConfig.extraMounts` 显式恢复。
- 模式显式原则：所有按需平台挂载和自定义挂载都必须显式声明 `mode: ro|rw`。
- 基础挂载覆盖原则：若只想修改 `/workspace`、`/root`、`/skills`、`/pan`、`/agent`、`/owner`、`/memory` 的模式，可在 `extraMounts` 中只写 `destination + mode` 覆盖默认模式，不新增第二个挂载。
...
- `platform: owner` 与 `platform: memory` 主要用于兼容旧配置或覆盖默认模式；若与默认挂载指向相同，不会新增第二个挂载。
- `platform: owner` 绑定的是 owner 目录；该目录内的正式 owner 文档位于 `owner/OWNER.md`。
- `platform: memory` 绑定的是当前 agent 的 memory 工作目录，容器内路径固定为 `/memory`，宿主路径为 `MEMORY_DIR/<agentKey>`。
- 默认基础挂载 `/workspace`、`/root`、`/skills`、`/pan`、`/agent`、`/owner`、`/memory` 可通过 `destination + mode` 覆盖默认模式。
