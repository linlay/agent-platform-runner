...
2. Adjust `.env` as needed. All `*_DIR` runtime directories default to `./runtime/*`, and you may override any of them to external absolute host paths. Local runs read these paths directly. Docker runs keep container paths fixed under `/opt/*`, and the bundle mounts `.env` to `/tmp/runner-host.env` so sandbox mounts can reuse the original host paths. If Container Hub runs on the host, keep `AGENT_CONTAINER_HUB_BASE_URL=http://host.docker.internal:11960`, and prefer real host-accessible paths in `*_DIR`. When you externalize the dynamic provider/model/MCP/viewport directories, prefer naming their shared parent `registries/`.
...
- recommended shared naming: keep static startup config in `configs/`, and use `registries/` as the parent for externalized `providers/`, `models/`, `mcp-servers/`, and `viewport-servers/`
