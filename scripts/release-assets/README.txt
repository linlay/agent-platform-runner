...
2. Adjust `.env` as needed. Runtime directories such as `AGENTS_DIR`, `OWNER_DIR`, `SKILLS_MARKET_DIR`, and `SCHEDULES_DIR` default to `./runtime/*`, and you may override them to any external host paths. The bundle mounts `.env` into the runner container so sandbox mounts can reuse these original host paths. If Container Hub runs on the host, keep `AGENT_CONTAINER_HUB_BASE_URL=http://host.docker.internal:11960`, and prefer real host-accessible paths in `*_DIR`.
