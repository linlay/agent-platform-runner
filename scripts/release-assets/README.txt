...
2. Adjust `.env` as needed. `SKILLS_MARKET_DIR` and `SCHEDULES_DIR` default to `./runtime/*`, and you may override them to any external host paths. If Container Hub runs on the host, keep `AGENT_CONTAINER_HUB_BASE_URL=http://host.docker.internal:11960`.
