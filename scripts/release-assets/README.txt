...
2. Adjust `.env` as needed. `REGISTRIES_DIR` defaults to `./runtime/registries`, while the other `*_DIR` runtime directories default to `./runtime/*`; you may override any of them to external absolute host paths. Local runs read these paths directly. Docker runs keep container paths fixed under `/opt/*`, and sandbox mounts read the original host paths directly from the container environment variables populated by `.env`. If Container Hub runs on the host, keep `AGENT_CONTAINER_HUB_BASE_URL=http://host.docker.internal:11960`, and prefer real host-accessible paths in `*_DIR`. The corresponding container registry paths are fixed under `/opt/registries/*`.
...
- no precreated `runtime/`: host runtime directories come from `.env`, `REGISTRIES_DIR` defaults to `./runtime/registries`, the rest default to `./runtime/*`, and all are auto-created by `./start.sh`
