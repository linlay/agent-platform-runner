...
- `GET /api/resource?...`：提供静态文件访问，支持 resource ticket 校验。
...
- `reasoning.snapshot`：`reasoningId`, `runId`, `text`, `taskId?`（仅历史数据展示；实时 SSE 不发送）
- `content.start`：`contentId`, `runId`, `taskId?`
- `content.delta`：`contentId`, `delta`
- `content.end`：`contentId`
- `content.snapshot`：`contentId`, `runId`, `text`, `taskId?`（仅历史数据展示；实时 SSE 不发送）
...
- `tool.snapshot`：`toolId`, `runId`, `toolName`, `taskId?`, `toolType?`, `toolLabel?`, `toolDescription?`, `viewportKey?`, `arguments?`（仅历史数据展示；为 `tool.start/tool.args/tool.end` 的合并视图，不含 `tool.result`）
...
- `action.snapshot`：`actionId`, `runId`, `actionName`, `taskId?`, `description?`, `arguments?`（仅历史数据展示；为 `action.start/action.args/action.end` 的合并视图，不含 `action.param` / `action.result`）
...
- `_seq` 标识 run 内的步骤顺序；历史读取与回放时按同一 `runId` 下的 `_seq` 升序拼接 step。
...
- `reasoningId` / `contentId` 默认从 `runId` 扩展生成：`{runId}_r_{seq}` / `{runId}_c_{seq}`。`seq` 从 `1` 开始，在同一 `chatId + runId` 下跨多个 step 连续递增，不会在新 step 里重置。
- `toolId` / `actionId` 默认不再单独生成随机前缀 ID。正常写入时直接复用 `toolCallId`：识别为 action 时写 `_actionId=toolCallId`，否则写 `_toolId=toolCallId`。
- 只有历史回放兜底、且既缺少外层 `_toolId/_actionId` 又缺少可复用 `toolCallId` 时，才会临时构造带 `runId` 的 fallback id；该 fallback 仅用于回放事件，不回写 chat storage。
- chat 内原始消息顺序规则：先按 run 分组；同一 run 内按 step 的 `_seq` 升序；同一 step 内保持 `messages[]` 原始写入顺序。
- `runId` 的时间戳特性：它来自 `System.currentTimeMillis()` 的 Unix Epoch 毫秒值，编码前不额外加随机数、机器号或序列号；因此它天然携带创建时间，可大致反推出 run 创建时刻。不同毫秒生成的 `runId` 通常随时间递增；但它依赖系统时钟，不保证严格单调，也不保证并发场景下同一毫秒内绝对唯一。

**ID 生成规则：**

| 工具类型 | 生成规则 |
|----------|---------|
| runId | `base36(epochMillis)`，仅含 `0-9a-z` |
| reasoningId | `{runId}_r_{seq}` |
| contentId | `{runId}_c_{seq}` |
| toolId | 默认直接复用 `toolCallId` |
| actionId | 默认直接复用 `toolCallId`（前提是该调用被识别为 action） |
| msgId | `m_` + 8 位 hex |
...
| ID 类型 | 前缀 |
|---------|------|
| reasoningId suffix | `_r_` |
| contentId suffix | `_c_` |
| msgId | `m_` |

SSE / 历史快照中的 `reasoningId` / `contentId` 格式：`{runId}_r_{seq}` / `{runId}_c_{seq}`。`toolId` / `actionId` 优先使用落盘值；若缺失则优先 fallback 到 `toolCallId`，最后才使用回放期临时构造的带 `runId` 标识。
